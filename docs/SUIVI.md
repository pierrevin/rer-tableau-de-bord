...
| 08/03/2026 | Migration Next 16 + proxy + sécurité auth renforcée | Lint/test/build OK, audit prod à 0 vulnérabilité |
| 08/03/2026 | Parcours “mot de passe oublié” + architecture mail provider-agnostique | Providers: webhook, resend, smtp, noop |
...
4. `npm run db:seed`  
5. `npm run dev` → ouvrir http://localhost:3000 et vérifier compteurs + `/api/articles`  
_Note_ : la commande `import:xlsx` a été retirée (dépendance vulnérable et script absent).
